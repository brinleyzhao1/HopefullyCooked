...
namespace StarterAssets
{
  ...
  public class ThirdPersonController : MonoBehaviour
  {
    ...
    private void Move()
    {
      ...
      {
        //We're not basing movement off camera angle, so just move in the input direction
        _targetRotation =
      ...
      //}
      ratAnimator.SetBool("isWalking", _input.move != Vector2.zero);
    }
    ...
    //Prevent Interact() from continuously being called
    private bool stopLoop = false;

    //Interacts with an object in the scene. Stopping interaction
    //just drops objects for now.
    private void CheckInteraction()
    {
      ...
      {
        // GetComponent<PlayerInteraction>().Drop(); //commented out to make interaction toggle
        stopLoop = false;
      }
    }

    //Currently does nothing, previously used to pick up ingredients.
    //It's been generalized to CheckInteraction()
    // private void PickUpOrDrop()
    // {
    // 	if (_input.pickUp)
    // 	{
    // 		//GetComponent<Player>().PickUpOrDrop();
    // 	}else{
    // 		//GetComponent<Player>().Drop();
    // 	}
    // }
